...
configParser = section "global" $ do
    ...
    mSingleMask <- fmap (buildModMask Xlib.noModMask)
        <$> fieldMbOf "close-single-mask" (listWithSeparator "-" string)
    mAllMask <- fmap (buildModMask Xlib.noModMask)
    ...
    where
        ...
        parseColor c =
            if "\"#" `T.isPrefixOf` c then
                T.reverse . T.drop 1 . T.reverse $ T.drop 1 c
            else
                c
