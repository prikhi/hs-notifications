...
grabKeyWithIgnoreMasks d k m w o p kb action =
    let
        ignoredModifiers =
            [Xlib.lockMask, Xlib.mod2Mask, Xlib.mod3Mask]
        baseKeyFunction =
            const $ withGrabKey d k m w o p kb action
        withIgnoredKeyMasks = foldl ignoredGrab baseKeyFunction (subsequences ignoredModifiers)
    in
        withIgnoredKeyMasks ()
    where
        ignoredGrab :: (() -> IO ()) -> [Xlib.KeyMask] -> (() -> IO ())
        ignoredGrab f masks _ =
            withGrabKey d k (m .|. foldl (.|.) Xlib.noModMask masks) w o p kb f
